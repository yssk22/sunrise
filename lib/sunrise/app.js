...
function makeApp(apppath, options, callback){
          ...
          if( exists ){
            try{
              ...
              if( typeof(handler) == 'function' ){
                handler(app);
                app.logger.info('hanlder initialized');
              }else{
                throw new Error('handler module must be a function');
              }
              app.use(staticProvider(path.join(apppath, '_attachments')));
              callback(undefined, app);
            }catch(e){
              callback(e);
            }
          }else{
            app.logger.info('handler not found');
            callback(undefined, app);
          }
        });
      }
  ...
  function processFile(fpath, fun){
    wait(function(done){
  ...
  processFile(options.messageDir, function(done){
    app.i18n.installMessages(options.messageDir, function(err){
      if( err ){
        callback(e);
      }else{
        app.logger.info('i18n resources are installed.');
        done();
      }
    });
  });

  processFile(options.modelPath, function(done){
    app.model = $.extend( { db: app.db, i18n: app.i18n },
                          require('./middleware/db'),
                          require(options.modelPath));
    app.logger.info('model extensions are installed.');
    done();
  });

  processFile(options.dhelperPath, function(done){
    app.dynamicHelpers(require(options.dhelperPath));
    app.logger.info('dynamieHelper extensions are installed.');
    done();
  });

  processFile(options.helperPath, function(done){
    app.helpers(require(options.helperPath));
    app.logger.info('helper extensions are installed.');
    done();
  });
}