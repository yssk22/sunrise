...
function setUp(){
  return site.createSite(__dirname + "/../fixtures/test_site");
}
...
module.exports = {
  ...
  "test serial": function(){
    ...
    function fun(req, res, next){
      res.bindings.foo += 1;
      res.bindings.foo_str += res.bindings.foo;
      setTimeout(next, 1);
    }
    app.get('/',
            function(req, res, next){
              res.bindings.foo = 0;
                  res.bindings.foo_str = "";
              next();
            },
            util.serial(
              fun,
    ...
    assert.response(app, {
      ...
      assert.ok(JSON.parse(res.body).foo, 3);
      assert.ok(JSON.parse(res.body).foo_str, "123");
    });
  }
};