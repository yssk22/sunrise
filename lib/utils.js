  ...
  if( target == null ){ // null or undefined
    ...
    for(var i in target){
      if( target.hasOwnProperty(i) ){
        ...
        ((k => {
          process.nextTick(() => {
            bindContext(target[k], context, (err, result) => {
              if( result === undefined ){
                delete(target[k]);
              }else{
                target[k] = result;
...
exports.logConfigure = (fileSystem, standardOutput, configPaths) => {
  log4js = require('log4js')(fileSystem, standardOutput, configPaths);
  return log4js;
}
 ...
 * @api public
 */
exports.loadJSON = (filepath, callback) => {
