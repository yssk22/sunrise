...
var dbMiddleware = require('./middleware/db');
...
exports.createSite = (sitepath, options) => {
  var logger = require('./utils').getLogger();
  var options = merge({}, require('./config'), options);
  // session impl
  ...
  site._installedApps = {};
  site.__defineGetter__('installedApps', () => // TODO: cachable
  Object.keys(site._installedApps).map(k => {
   ...
   */
  site.install = (apppath, rootUrl, options) => {
    options = merge({
      rootUrl,
    ...
    Object.keys(site._installedApps).forEach(k => {
      var app = site._installedApps[k];
  ...
  site.deployAll = function(cb){
    var apps = Object.keys(site._installedApps).map(k => site._installedApps[k]);
    apps.unshift(site);
    var funs = apps.map(a => a.deploy);
    funs.push(cb);
